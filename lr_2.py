...
class Evo2:
    ...
    def fun3(self, x):
        return (3*x[0] - 2*x[1] - 1)**2/175 + (-x[0] + 2*x[1])**2/17 - 13
    ...
    def select(self, fun, arr):
        ...
        sorted_array = np.array(sortes_ls)
        sorted_array.reshape((-1,1))
        return np.reshape(sorted_array, (-1, 2))
    ...
    def get_z(self, fun, generation):
        arr = np.array([fun(e) for e in generation])
        return arr.reshape((-1,1))

    def z(self, generation):
        #x**2
        z1 = self.get_z(self.fun1, generation)
        z2 = self.get_z(self.fun2, generation)
        z3 = self.get_z(self.fun3, generation)
        z4 = z1 + z2 + z3

        return np.hstack((z1, z2, z3, z4))


evo = Evo2(500, max_generation=5000)
final_gen = evo.evolution()
z = evo.z(final_gen)
